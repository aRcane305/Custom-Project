...
class MusicPlayerMain < Gosu::Window
  ...
  def display_tracks(tracks)
    ...
    while index < @tracks.length
      ...
      @primary_font.draw_text(track_name, TRACK_TEXT_X_OFFSET, y_coord, ZOrder::UI, 1.0, 1.0, Gosu::Color::BLACK)
      index += 1
    end
  end
  ...
  def draw_albums
    ...
    while index < @albums.length
      ...
      album.album_cover.bmp.draw(x_coord, y_coord, ZOrder::UI_BACKGROUND, 1, 1)
  ...
  def button_down(id)
    ...
    when Gosu::MsLeft
      ...
      while index < @albums.length
        ...
        x_coord = ALBUM_START_X + ((index % 2) * (width + spacing))
        y_coord = ALBUM_START_X + ((index / 2) * (height + spacing))
